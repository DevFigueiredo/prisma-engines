                    ...
                    if let Some(sort_aggregation) = extract_sort_aggregation(inner_field_name.as_ref()) {
                        ...
                        match related_field {
                            Field::Scalar(sf) => {
                                ...
                                Ok(Some(OrderBy::to_many_field(path, sf, sort_order, nulls_order)))
                            }
                            _ => unreachable!("To-many relation orderBy inner field must be a scalar or an aggregation function."),
                        }
                    }
                }
